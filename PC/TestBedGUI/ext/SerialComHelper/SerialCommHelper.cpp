...
CSerialCommHelper::CSerialCommHelper()
{
	...
	m_DebugFile.Init(("c:\\Temp\\") ,("SerialIO"),("RCSERIAL:"));
}
...
HRESULT CSerialCommHelper:: Init(std::string szPortName, DWORD dwBaudRate,BYTE byParity,BYTE byStopBits,BYTE byByteSize)
{
	...
	try
	{
		...
		//open the COM Port
		m_hCommPort = ::CreateFile((LPCWSTR)szPortName.c_str (),
		...
		case WAIT_OBJECT_0 + 1:
			{
				...
				ATLTRACE6((LPCSTR)_T("RCSerial: Q Unlocked:"));
				if ( iAccum > 0 )
				{
					ATLTRACE6((LPCSTR)_T("CSerialCommHelper(worker thread):  SetDataReadEvent() len:{%d} data:{%s}"),iAccum,(apThis->m_theSerialBuffer.GetData()).c_str ()  );
...
HRESULT CSerialCommHelper::Write (const char* data,DWORD dwSize)
{
	...
	ATLTRACE6((LPCSTR)_T("RCSerial:Writing:{%s} len:{%d}"),(szData).c_str(),szData.size());
...
HRESULT CSerialCommHelper::Read_Upto	(std::string& data,char chTerminator ,long* alCount,long alTimeOut)
{
	...
	{
		ATLTRACE6((LPCSTR)_T("CSerialCommHelperUnhandled exception"));
		ASSERT ( 0  ) ;
	}
	return hr;
...
HRESULT CSerialCommHelper::Read_N		(std::string& data,long alCount,long  alTimeOut )
{
	...
	{
		ATLTRACE6((LPCSTR)_T("CSerialCommHelper Unhandled exception"));
		ASSERT ( 0  ) ;
	}
	return hr;
...
HRESULT CSerialCommHelper::ReadAvailable(std::string& data)
{
	...
	{
		ATLTRACE6((LPCSTR)_T("CSerialCommHelper Unhandled exception in ReadAvailable()"));
		ASSERT ( 0  ) ;
		hr = E_FAIL;
	}
	return hr;
